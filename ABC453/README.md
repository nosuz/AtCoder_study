## 開催日

2026-04-11

## コンテストページ

[AtCoder Beginner Contest 453](https://atcoder.jp/contests/abc453)

## [A - Trimo](https://atcoder.jp/contests/abc453/tasks/abc453_a)

正規表現による置換。

- Python

## [B - Sensor Data Logging](https://atcoder.jp/contests/abc453/tasks/abc453_b)

前の値を記憶して起きて、絶対値を調べる。

- Python

## [C - Sneaking Glances](https://atcoder.jp/contests/abc453/tasks/abc453_c)

bit全探索。単純に0を超えたら逆方向に移動ではなかった。

スタート位置が0.5なので、全体を2倍すれば整数演算にできる。

- Python

## [D - Go Straight](https://atcoder.jp/contests/abc453/tasks/abc453_d)

## [E - Team Division](https://atcoder.jp/contests/abc453/tasks/abc453_e)

## [F - Avoid Division](https://atcoder.jp/contests/abc453/tasks/abc453_f)
